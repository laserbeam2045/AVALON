...
*/
static void clearDrops(Board *board, uint64_t bitCombo, ComboData *comboData, ClearingSettings *csp)
{
  uint64_t checked = 0;
  ...
  for (char index = CLEAR_START_INDEX; index <= CLEAR_END_INDEX; index++) {
    ...
    case 4:
      if (ClearingSettings_isActiveOf(csp, (CS_TYPE)TWO_WAY, color)) {
        ComboData_incrementClearStyle(comboData, (CS_TYPE)TWO_WAY, color);
      }
      break;
    case 5:
      if (
        ClearingSettings_isActiveOf(csp, (CS_TYPE)L, color) &&
    ...
    default:
      if (
        ...
        ClearingSettings_isActiveOf(csp, (CS_TYPE)BREAK_THROUGH, color) &&
        is3x3(clearIndices, clearCount)
      ) {
        ComboData_incrementClearStyle(comboData, (CS_TYPE)BREAK_THROUGH, color);
      }
    }
  }
}
